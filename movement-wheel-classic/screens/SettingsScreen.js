...
export default function SettingsScreen({ navigation }) {
  ...
  const clearAllLocalData = async () => {
    try {
      await clearHistory(); // clear stored sessions
      startNewSession(); // reset draft
      setSavedCount(0); // update UI
  ...
  const goToHomeTab = () => {
    // Prefer parent tab nav, fallback to plain navigate.
    const parent = navigation.getParent?.();
    if (parent?.navigate) parent.navigate("Home");
  ...
  return (
    <AppShell title="Settings">
      <View>
        ...
        <Button title="Go to Home" onPress={goToHomeTab} />
        ...
        <Button
          ...
          onPress={() => {
            Alert.alert(
              "Clear all data?",
              "This will delete saved sessions and reset the current draft on this device.",
              [
